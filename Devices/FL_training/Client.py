...
class Client(Communicator):
	...
	def initialize(self, split_layer, offload, first, LR):
		...
		#save initial model weights for clients
		torch.save(self.net.state_dict(),'./initial_model_weights.pth')
	...
	def train(self, trainloader, epoch):
		...
		if self.split_layer == (config.model_len -1): # No offloading training
			...
			while should_restart:
				...
				#for batch_idx, (inputs, targets) in enumerate(tqdm.tqdm(trainloader)):
				for batch_idx in tqdm.tqdm(range(epochs)):
					correct = 0
					...
					msg_tmp = self.recv_msg(self.sock)
					if msg_tmp[0] == 'MIGRATION_TO': #update server ip in client
						self.reset_connection()
						#time.sleep(3.4)
						logger.info('Connecting to Server2 192.168.10.104')
						self.sock.connect(('192.168.10.104',51000))
						print('connected')
						#wait for gradients from new server
						#gradients = self.recv_msg(self.sock)[1].to(self.device)
						gradients = self.recv_msg(self.sock)[1]
						print(gradients)
						print('gradients received')
					else:
						gradients = msg_tmp[1].to(self.device)

					outputs.backward(gradients)
					self.optimizer.step()
					
					#simulate migration
					if batch_idx in its and repeat_once == True and epoch in epochs_list:
						#self.net.apply(self.weight_reset)
						#self.net.load_state_dict(torch.load('./initial_model_weights.pth'))
						should_restart = True
						repeat_once = False
						temp_it = batch_idx + 1
						temp_epoch = epoch
					...
					if epoch == temp_epoch and batch_idx == temp_it and temp_it is not None:
						repeat_once = True
						temp_it = None
						temp_epoch = None

					if batch_idx == epochs-1:
	...
	def upload(self):
		...
		self.send_msg(self.sock, msg)
		print(msg)
		print('server to client msg sent')
