'''Some helper functions for FedFly, including:
...
def get_local_dataloader(CLIENT_IDEX, cpu_count):
	...
	print(int((N/K) * CLIENT_IDEX))
	print(int((N/K) * (CLIENT_IDEX+1)))
	print(CLIENT_IDEX)
...
def send_initial_model(location, model_name, layer, device, cfg, socks, edge_server_socks):
	cfg = cfg.copy()
	net = VGG(location, model_name, layer, cfg)
	net = net.to(device)
	logger.debug(str(net))
	#send initial weights to the edge servers
	print('sending initial weights to the edge servers')
	msg = ['UNIT_MODEL_FROM_SERVER', net.state_dict()]
	socks.send_msg(edge_server_socks, msg)	
	

def get_model_from_server(location, model_name, layer, device, cfg, sock, edge_server_sock):
	cfg = cfg.copy()
	net = VGG(location, model_name, layer, cfg)
	net = net.to(device)
	logger.debug(str(net))
	msg = self.recv_msg(socks, 'UNIT_MODEL_FROM_SERVER')
	net.load_state_dict(msg[1])
	return net
...
def concat_weights(weights,cweights,sweights=None):
...
def fed_avg(zero_model, w_local_list, total_data_size):
	#w_local_list = tuple(map(operator.add, w_local_list[0], w_local_list[1]))
	
	keys = w_local_list[0][0].keys()
	#keys2 = w_local_list[1][0].keys()
	#print(keys)
	#print(keys2)
	w0 = w_local_list[0]
	#w1 = w_local_list[1]
	for k in keys:
		...
		for w in w_local_list:
			print(w[1])
		#beta = (float(w0[1]) + float(w1[1])) / float(totoal_data_size)
			#beta = (float(w[1])) / float(totoal_data_size)
			beta = (float(w[1])) / float(total_data_size)
			if 'num_batches_tracked' in k:
				#zero_model[k] = w0[0][k] + w1[0][k]
				zero_model[k] = w[0][k] 
			else:	
				#zero_model[k] += ((w0[0][k] + w1[0][k]) * beta)
