...
class Sever(Communicator):
	...
	def _thread_training_offloading(self, client_ip, epoch):
		...
		iteration = int((self.datasetsize / ((config.K) * config.B)))
		print(iteration)
		migration_sender = False
		run_network = True
		results = {}
		results['Accuracy'] = []
		epochs_list = [10,20,30,40,50,60,70,80,90]
		its = [90]
		temp_it = None
		temp_epoch = None

		torch.save(self.nets[client_ip].state_dict(),'./initial_model_weights.pth')

		if (threading.currentThread().getName() == '192.168.101.107'):
			repeat_once = True
			should_restart = True
			while should_restart:
				...
				for i in range(iteration):
					...
					if run_network:
						...
						if(str(client_ip) == '192.168.101.107') and epoch in epochs_list and i in its and repeat_once == True:
							#self.nets['192.168.101.107'].apply(self.weight_reset)
							self.nets['192.168.101.107'].load_state_dict(torch.load('./initial_model_weights.pth'))
							should_restart = True
							repeat_once = False
							print('restart')
							temp_it = i + 1
							...
							self.send_msg(self.client_socks[client_ip], msg)
							break
						if epoch == temp_epoch and i == temp_it and temp_it is not None:
							repeat_once = True
							temp_it = None
							temp_epoch = None
			...
			for i in range(iteration):
				...
				if run_network:
					...
					acc = 100.*correct/targets.size(0)
					#print(str(i) + ' ' + str(client_ip) + ' ' + str(acc))
					#if(str(client_ip) == '192.168.10.31') and epoch == 2 and i == 60:
					#	self.nets['192.168.10.109'].apply(self.weight_reset)
					#	should_restart = True
					#	repeat_once = False
					#	print('restart')
					#	results['Accuracy'].append(acc)
	...
	def aggregate(self, client_ips):
		...
		for i in range(len(client_ips)):
			...
			if config.split_layer[i] != (config.model_len -1):
				w_local = (utils.concat_weights(self.uninet.state_dict(),msg[1],self.nets[client_ips[i]].state_dict()),self.datasetsize / config.K)
	...
	def collectWeights(self, client_ips):
		...
		for i in range(len(client_ips)):
			...
			if config.split_layer[i] != (config.model_len -1):
				...
				send_data[client_ips[i]]['N'] = self.datasetsize / config.K
		...
		while check_connection:
			try:
				...
				print('increasing waiting time for weights for 10 seconds')
				t.sleep(10)
	...
	def reinitialize(self, split_layers, offload, first, LR):
		...
		wait_start_time = t.time()
		print('Start waiting for other edge server to complete at: '+ str(wait_start_time))
		t.sleep(10)
		wait_end_time = t.time()
		print('total waiting time is:' + str(wait_end_time - wait_start_time))
		self.reset_connection()
		self.get_data_from_unitserver()
		t.sleep(3.5)
